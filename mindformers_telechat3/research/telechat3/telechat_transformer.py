...
class TelechatAttention(nn.Cell):
    ...
    def __init__(self,
        ...
        self.sp = parallel_config.context_parallel
        self.shape = P.Shape()
        self.cast = P.Cast()
        ...
        if self.qkv_concat:
            ...
            if self.qkv_has_bias:
                self.w_qkv.shard(((dp * self.sp, 1), (mp, 1)), ((dp * self.sp, mp), (mp,)))
            else:
                self.w_qkv.shard(((dp * self.sp, 1), (mp, 1)))
            self.split_qkv = ms.ops.auto_generate.SplitWithSize()
            self.split_qkv.add_prim_attr("skip_redistribution", True)
            self.split_qkv.shard(((dp * self.sp, 1, mp),))
        else:
            ...
            if qkv_has_bias:
                ...
                self.wk.shard(((dp * self.sp, 1), (mp, 1)), ((dp * self.sp, mp), (mp,)))
                self.wv.shard(((dp * self.sp, 1), (mp, 1)), ((dp * self.sp, mp), (mp,)))
            else:
                self.wq.shard(((dp * self.sp, 1), (mp, 1)))
                self.wk.shard(((dp * self.sp, 1), (mp, 1)))
                self.wv.shard(((dp * self.sp, 1), (mp, 1)))
        ...
        if out_proj_has_bias:
            self.wo.shard(((dp * self.sp, mp), (1, mp)), ((dp * self.sp, 1), (1,)), out_strategy_matmul=((dp * self.sp, 1),))
        else:
            self.wo.shard(((dp * self.sp, mp), (1, mp)), out_strategy_matmul=((dp * self.sp, 1),))
        ...
        if self.use_past:
            ...
            if not (_get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation()):
                self.transpose.shard(((dp, self.sp, mp, 1),))
                layout = Layout((dp, self.sp, mp), ("dp", "sp", "mp"))
                layout_merger_transpose = (layout("dp", "mp", "sp", "None"),)
                self.merger_head_transpose.shard(in_strategy=layout_merger_transpose)
                ...
                if parallel_config.use_seq_parallel and self.is_first_iteration:
                    self.wo.shard(((dp * self.sp, mp), (1, mp)), out_strategy_matmul=((dp * mp * self.sp, 1),))
            ...
            if self.use_flash_attention:
                self.input_layout = "BSH" if self.sp > 1 else "BNSD"
...
class TelechatDecodeLayer(nn.Cell):
    ...
    def __init__(self,
        ...
        parallel_config_new.data_parallel *= parallel_config.model_parallel * parallel_config.context_parallel
        ...
        if parallel_config.use_seq_parallel and self.is_first_iteration:
            if self.expert_num == 1:
                self.add.shard(((dp, mp * sp, 1), (dp, mp * sp, 1)))
                self.attention_norm.shard((dp, mp * sp, 1))
                self.ffn_norm.shard((dp, mp * sp, 1))
                self.feed_forward.w2.shard(((dp * sp, mp), (1, mp)), ((dp * mp * sp, 1), (1, )),
                                       out_strategy_matmul=((dp * mp * sp, 1),))
            else:
                self.add.shard(((dp, mp * sp, 1), (dp, mp * sp, 1)))
                self.attention_norm.shard((dp, mp * sp, 1))
                self.ffn_norm.shard((dp, mp * sp, 1))
    ...
    def construct(self, x, freqs_cis, mask=None, batch_valid_length=None, block_tables=None,
        ...
        # [bs, seq/1, hidden_dim] or [bs * seq/1, hidden_dim]
        out = self.add(self.cast(h, self.res_dtype), self.cast(ffn_out, self.res_dtype))
