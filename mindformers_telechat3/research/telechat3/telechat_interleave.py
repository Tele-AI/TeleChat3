...
from mindformers.modules.layers import _check_input_dtype, RotaryEmbedding
from mindformers.modules.transformer import TransformerOpParallelConfig
from mindformers.modules.flash_attention import FlashAttention

from research.telechat3.telechat_layer import TelechatLinear, TelechatFeedForward
...
class TelechatAttentionInterleave(nn.Cell):
    ...
    def __init__(self,
        ...
        self.sp = sp
        self.split_kv.shard(((dp * sp, mp, 1),))
        if not (_get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation()):
            self.transpose.shard(((dp, sp, mp, 1),))
            if sp > 1:
                layout = Layout((dp, sp, mp), ("dp", "sp", "mp"))
                layout_merge_head_transpose = (layout("dp", "mp", "sp", "None"),)
                self.merger_head_transpose.shard(in_strategy=layout_merge_head_transpose)
            else:
                self.merger_head_transpose.shard(((dp, mp, 1, 1),))
            self.batch_matmul_q_k.shard(((dp, mp, sp, 1), (dp, mp, sp, 1)))
            self.batch_matmul.shard(((dp, mp, sp, 1), (dp, mp, sp, 1)))
            self.mul.shard(((dp, mp, sp, 1), ()))
            self.add.shard(((dp, 1, 1, 1), (dp, mp, sp, 1)))
            self.softmax.shard(((dp, mp, 1, 1),))
            self.tile_kv.shard(((dp, mp, 1, sp),))

            self.apply_rotary_emb.shard(parallel_config)

            if self.qkv_has_bias:
                self.wq.shard(((dp * sp, 1), (mp, 1)), ((dp * sp, mp), (mp,)))
                self.wk.shard(((dp * sp, 1), (mp, 1)), ((dp * sp, mp), (mp,)))
                self.wv.shard(((dp * sp, 1), (mp, 1)), ((dp * sp, mp), (mp,)))
            else:
                self.wq.shard(((dp * sp, 1), (mp, 1)))
                self.wk.shard(((dp * sp, 1), (mp, 1)))
                self.wv.shard(((dp * sp, 1), (mp, 1)))
            if self.out_proj_has_bias:
                self.wo.shard(((dp * sp, mp), (1, mp)), ((dp * sp, 1), (1,)))
            else:
                self.wo.shard(((dp * sp, mp), (1, mp)))
            if parallel_config.use_seq_parallel and self.is_first_iteration and sp==1:
                if self.out_proj_has_bias:
        ...
        if self.use_flash_attention:
            self.input_layout = "BSH" if sp > 1 else "BNSD"
...
class TelechatDecodeLayerInterleave(nn.Cell):
    ...
    def __init__(self,
        ...
        if not (_get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation()):
            ...
            self.feed_forward.mul.shard(((dp * sp, mp), (dp * sp, mp)))
            self.add.shard(((dp * sp, 1), (dp * sp, 1)))
            if sp > 1:
                self.attention_norm.shard((dp * sp * mp, 1))
                self.ffn_norm.shard((dp * sp * mp, 1))
            else:
                self.attention_norm.shard((dp, 1))
                self.ffn_norm.shard((dp, 1))

        if parallel_config.use_seq_parallel and self.is_first_iteration:
            self.add.shard(((dp * sp * mp, 1), (dp * sp * mp, 1)))
            self.attention_norm.shard((dp * sp * mp, 1))
            self.ffn_norm.shard((dp * sp * mp, 1))
            self.feed_forward.w2.shard(
                    strategy_matmul=((dp * sp, mp), (1, mp)),
                    strategy_bias=((dp * sp * mp, 1), (1, )),
                    out_strategy_matmul=((dp * sp * mp, 1),))
        ...
        for _ in range(self.interleave_num):
            ...
            if parallel_config.use_seq_parallel:
                if self.layer_id == self.num_layers - 2:
                    concat_stra2.append((dp * sp, 1))
                else:
                    concat_stra2.append((dp * mp * sp, 1))
                if self.layer_id == self.num_layers - 1:
                    interleave_data1.strided_slice_list[0].shard(((dp * sp, 1),))
                else:
                    interleave_data1.strided_slice_list[0].shard(((dp * mp * sp, 1),))
                interleave_data1_.strided_slice_list[0].shard(((1, 1),))
                interleave_data2.strided_slice_list[0].shard(((dp * mp * sp, 1),))
            else:
                concat_stra2.append((dp * sp, 1))
                interleave_data1.strided_slice_list[0].shard(((dp * sp, 1),))
                interleave_data1_.strided_slice_list[0].shard(((1, 1),))
                interleave_data2.strided_slice_list[0].shard(((dp * sp, 1),))
            ...
            interleave_data2.strided_slice_list[1].shard(((dp * sp, mp),))
