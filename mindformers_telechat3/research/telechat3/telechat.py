...
class AttentionMaskWithEod(nn.Cell):
    def __init__(self, seq_length, parallel_config=default_dpmp_config, compute_dtype=mstype.float16,
        ...
        super(AttentionMaskWithEod, self).__init__()
        ...
        # lower triangle for attention mask
        #self.lower_triangle_mask = Tensor(np.tril(np.ones(shape=(1, seq_length, seq_length))), mstype.int16)
    ...
    def construct(self, sequence_start_ids):
        ...
        combined_mask = self.mul(self.cast(lower_triangle_mask, self.compute_dtype), self.cast(doc_mask, self.compute_dtype))
        return combined_mask
        #check = self.less(self.cast(self.expand_dims(sequence_start_ids, -1), mstype.int16), self.arange_tensor)
        #res = self.cast(self.mul(self.lower_triangle_mask, check), self.compute_dtype)
        #return res
...
class TelechatModel(TelechatPreTrainedModel):
    ...
    def construct(self, tokens: Tensor, batch_valid_length=None, batch_index=None, zactivate_len=None, aux_loss=None,
        ...
        if self.expert_num > 1:
            return output, embedding_weight, aux_loss
...
class TelechatForCausalLM(TelechatPreTrainedModel):
    ...
    def prepare_inputs_for_predict_layout(self, input_ids, **kwargs):
        ...
        return input_ids, labels, None, None, None, None, None, None, None, None, None, None, slot_mapping, prefix_keys_values
    ...
    # pylint: disable=W0613
    def construct(self, input_ids, labels=None, loss_mask=None, input_position=None, position_ids=None, attention_mask=None,
        ...
        if self.expert_num == 1:
            output, embedding_weight = self.model(tokens, batch_valid_length, batch_index, zactivate_len, block_tables=block_tables, \
                                                  attention_mask=attention_mask, slot_mapping=slot_mapping, prefix_keys_values=prefix_keys_values)
        else:
            output, embedding_weight, aux_loss = self.model(tokens, batch_valid_length, batch_index, zactivate_len, self.init_aux_loss,
                                                            block_tables, attention_mask, slot_mapping, prefix_keys_values)
