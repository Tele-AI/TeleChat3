...
from mindformers.experimental.infer.core.transformer import \
    ParallelAttention, ParallelTransformerLayer, ParallelTransformer, ParallelMLP, VocabEmbedding
...
from mindformers.modules.layers import FreqsMgrDynamicNTK, FreqsMgrDynamicYarn
...
class TelechatParallelAttention(ParallelAttention):
    ...
    Supported Platforms:
        ``Ascend``
    """
    def __init__(self, config, layer_number, attention_type="self_attn", attn_mask_type=None, wq=None, wk=None, wv=None, wo=None, w_qkv=None):
        super().__init__(config, layer_number, attention_type, attn_mask_type)
        if wq is not None:
            assert (wk is not None) and (wv is not None) and (wo is not None)
            self.wq = wq
            self.wk = wk
            self.wv = wv
            self.wo = wo
        elif w_qkv is not None:
            self.w_qkv = w_qkv
            self.wo = wo
...
class TelechatParallelTransformerLayer(ParallelTransformerLayer):
    ...
    def __init__(
        ...
        # Attention.
        self.attention = TelechatParallelAttention(config, layer_number, wq=wq, wk=wk, wv=wv, wo=wo, w_qkv=w_qkv)
        # MLP
...
class TelechatParallelTransformer(ParallelTransformer):
    ...
    def __init__(
        ...
        self.enable_dynamic_freqs = False
        if config.extend_method == 'DYNAMIC_NTK':
            self.enable_dynamic_freqs = True
        ...
        elif config.extend_method == 'DYNAMIC_YARN':
            self.enable_dynamic_freqs = True
            self.freqs_mgr = FreqsMgrDynamicYarn(head_dim=self.head_dim,
                                                 seq_length=config.seq_length,
                                                 rotary_dtype=config.rotary_dtype,
                                                 theta=config.theta,
                                                 scaling_factor=config.scaling_factor,
                                                 is_dynamic=config.is_dynamic)
            logger.info("Running with dynamic YARN.")

        self.embedding_size = config.embedding_size if self.model_name != 'telechat3_36b' else config.hidden_size
        ...
        if self.model_name != 'telechat3_36b':
            # self.embedding_hidden_mapping_in = ColumnParallelLinear(self.embedding_size,
            #                                                         config.hidden_size,
            #                                                         config=self.config.parallel_config,
            #                                                         bias=False,
            #                                                         gather_output=True,
            #                                                         transpose_b=True,
            #                                                         aram_init_type=self.config.param_init_dtype,
            #                                                         compute_dtype=self.config.compute_dtype)
            # self.embedding_hidden_mapping_out = RowParallelLinear(config.hidden_size,
            #                                                       self.embedding_size,
        ...
        for layer_id in range(config.num_layers):
            ...
            layer = TelechatParallelTransformerLayer(config=self.config, layer_number=layer_id + 1, wq=wq, wk=wk, wv=wv, wo=wo, w_qkv=w_qkv)
    ...
    def construct(self, tokens: Tensor, batch_valid_length=None, batch_index=None, zactivate_len=None,
        ...
        if self.model_name != 'telechat3_36b':
            hidden_states = self.embedding_hidden_mapping_out(hidden_states)
        if self.post_norm:
