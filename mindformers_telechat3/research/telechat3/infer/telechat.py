...
from mindspore import Tensor, ops, mint, mutable, nn
...
from research.telechat3.infer.telechat_transformers import TelechatParallelTransformer
...
class ParallelTelechatForCausalLM(TelechatPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.model_name != 'telechat3_36b':
            self.embedding_weight = self.model.tok_embeddings.embedding_weight.value()
            if config.parallel_config.vocab_emb_dp:
                self.lm_head = TelechatHead(
    ...
    def construct(self, input_ids, labels=None, input_position=None, position_ids=None, attention_mask=None,
        ...
        if self.model_name != 'telechat3_36b':
            logits = self.lm_head(output, self.model.tok_embeddings.embedding_weight.value())
        else:
