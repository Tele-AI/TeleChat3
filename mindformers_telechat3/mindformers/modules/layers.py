...
class FreqsMgrDynamicYarn(Cell):
    r"""freqs_cis manager."""
    ...
    def __init__(self,
        ...
        base = theta
        original_max_position_embeddings = scaling_factor["original_max_position_embeddings"]
        self.mscale_all_dim = scaling_factor["mscale_all_dim"]
        self.mscale_ = scaling_factor["mscale"]
        self.low, self.high = _yarn_find_correction_range(beta_fast, beta_slow, head_dim, base,
                                                          original_max_position_embeddings)
        self.inv_freq_mask = 1.0 - _yarn_linear_ramp_mask(self.low, self.high, head_dim // 2)
        self.inv_freq_mask = Tensor(self.inv_freq_mask, dtype=mstype.float32)
        self.swap_mask = Tensor(swap_mask, dtype=rotary_dtype)
        ...
        self.original_max_position_embeddings_inverse = 1 / original_max_position_embeddings
        ...
        self.reshape = P.Reshape()
        self.mul_freqs = P.Mul()
        self.tile = P.Tile()
        self.slice = P.StridedSlice()
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL, ParallelMode.SEMI_AUTO_PARALLEL):
            self.mul_freqs.shard(((1, 1), (1, 1)))
            self.tile.shard(((1, 1),))
            self.slice.shard(((1, 1),))

    def _yarn_get_mscale_ms(self, scale, mscale=1):
        mscale = self.mul(0.1, mscale)
        mscale = self.mul(mscale, mint.log(scale))
        mscale = self.add(mscale, 1.0)
        return mscale

    def get_dynamic_yarn_freqs(self, seq_length, seq_arange):
        extra_theta = self.mul(self.base, mint.ones(seq_length.shape, dtype=mstype.float32))
        extra_theta = self.reshape(extra_theta, (-1, 1))
        extra_freq = mint.pow(extra_theta, self.freqs_base)

        factor = self.mul(seq_length, self.original_max_position_embeddings_inverse)
        factor = self.reshape(factor, (-1, 1))
        factor_reciprocal = mint.pow(factor, -1)
        internal_freq = self.mul(extra_freq, factor_reciprocal)

        freqs = self.add(self.mul(internal_freq, self.sub(1.0, self.inv_freq_mask)),
                         self.mul(extra_freq, self.inv_freq_mask))

        if self.mscale_ is not None and self.mscale_all_dim is not None:
            mscale = self.div(self._yarn_get_mscale_ms(factor, self.mscale_),
                              self._yarn_get_mscale_ms(factor, self.mscale_all_dim))
        else:
            mscale = self._yarn_get_mscale_ms(factor)
    ...
    def prefill(self, bs, seq_length):
        ...
        seq_length = ops.clip_by_value(seq_length, clip_value_min=self.original_max_position_embeddings)
        seq_arange = ops.arange(start=0, end=seq_length, step=1)
        freqs_cos, freqs_sin = self.get_dynamic_yarn_freqs(seq_length, seq_arange)
    ...
    def increment(self, batch_valid_length):
        ...
        batch_valid_length = ops.clip_by_value(batch_valid_length,
                                               clip_value_min=self.original_max_position_embeddings)
        freqs_cos, freqs_sin = self.get_dynamic_yarn_freqs(batch_valid_length, indices)
        return freqs_cos, freqs_sin, self.swap_mask
