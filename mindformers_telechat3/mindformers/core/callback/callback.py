...
class MFLossMonitor(Callback):
    ...
    def __init__(self,
        ...
        self.wandb_init = True if wandb_config else False
        if self.wandb_init and self.rank_id == self.device_num - 1:
            logger.info(f"wandb_config: {wandb_config}")
            wandb.login(key=wandb_config["key"], host=wandb_config["host"], relogin=True)
        ...
        if self.tensor_writer is not None:
            ...
            if self.tensorboard.get('log_timers_to_tensorboard', False):
                ...
                self.tensor_writer.add_scalar('throughput vs samples', throughput,
                                              global_step=global_step * self.global_batch_size)
        if self.wandb_init and self.rank_id == self.device_num-1:
            wandb.log({"loss": loss, "step_time": int(per_step_seconds),"scale": scaling_sens,
                      "norm": global_norm, "lr": float(current_lr)}, step=(int(cur_epoch_num)-1)*int(steps_per_epoch)+int(cur_step_num))
...
class CheckpointMonitor(ModelCheckpoint):
    ...
    def save_checkpoint_network(self, cb_params):
        ...
        if self.save_trainable_params:
            ...
            save_obj.init_parameters_data(auto_parallel_mode=True)
